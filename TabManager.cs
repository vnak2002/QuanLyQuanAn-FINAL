...
namespace QuanLyQuanAn_FINAL.DAO
{
    public partial class TableManager : Form
    {
        ...
        {
            lsvBill.Items.Clear();
            List<QuanLyQuanAn_FINAL.DTO.Menu> listBillInfo = MenuDAO.Instance.GetListMenuByTable(id);
            float totalPrice = 0;
            foreach (QuanLyQuanAn_FINAL.DTO.Menu item in listBillInfo)
            {
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            if (idBill != -1)
            {
                if (MessageBox.Show(string.Format("Hóa đơn cho bàn {0}\nTổng tiền: {1}", table.Name, finalTotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                {
